...
final class ContentNormalizer: Sendable {
    ...
    nonisolated func normalize(from pasteboard: NSPasteboard) -> ClipboardItem? {
        ...
        // Transient content detection — skip concealed/transient/auto-generated items
        let concealedType = NSPasteboard.PasteboardType("org.nspasteboard.ConcealedType")
        let transientType = NSPasteboard.PasteboardType("org.nspasteboard.TransientType")
        let autoGeneratedType = NSPasteboard.PasteboardType("org.nspasteboard.AutoGeneratedType")
        // Proprietary markers per nspasteboard.org
        let proprietaryTransientTypes: [NSPasteboard.PasteboardType] = [
            .init("de.petermaurer.TransientPasteboardType"),
            .init("com.typeit4me.clipping"),
            .init("Pasteboard generator type"),
        ]
        let proprietaryConfidentialTypes: [NSPasteboard.PasteboardType] = [
            .init("com.agilebits.onepassword"),
            .init("net.antelle.keeweb"),
        ]
        let typeSet = Set(types)
        let skipTypes = Set([concealedType, transientType, autoGeneratedType])
            .union(proprietaryTransientTypes)
            .union(proprietaryConfidentialTypes)
        if !typeSet.isDisjoint(with: skipTypes) {
            return nil
        }
        ...
        if let richTextItem = extractRichText(
            from: pasteboard, sourceApp: sourceApp, maxSize: maxSize)
        {
            return richTextItem
        }
    ...
    private nonisolated func extractPlainText(
        from pasteboard: NSPasteboard, sourceApp: AppInfo, maxSize: Int
    ) -> ClipboardItem? {
    ...
    private nonisolated func extractRichText(
        from pasteboard: NSPasteboard, sourceApp: AppInfo, maxSize: Int
    ) -> ClipboardItem? {
        guard let rtfData = pasteboard.data(forType: .rtf) ?? pasteboard.data(forType: .html) else {
            return nil
        }
    ...
    private nonisolated func extractImage(
        from pasteboard: NSPasteboard, sourceApp: AppInfo, maxSize: Int
    ) -> ClipboardItem? {
    ...
    private nonisolated func extractFiles(
        from pasteboard: NSPasteboard, sourceApp: AppInfo, maxSize: Int
    ) -> ClipboardItem? {
        guard
    ...
    private nonisolated func extractURL(
        from pasteboard: NSPasteboard, sourceApp: AppInfo, maxSize: Int
    ) -> ClipboardItem? {
    ...
    private nonisolated func extractColor(from pasteboard: NSPasteboard, sourceApp: AppInfo)
        -> ClipboardItem?
    {
