    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            Image(systemName: "hand.raised.circle.fill")
                ...
                .symbolEffect(.pulse, options: .repeating, isActive: !viewModel.accessibilityGranted)

            Text("Accessibility Permission")
                .font(.system(size: 24, weight: .bold, design: .rounded))

            Text("OpenPaste needs Accessibility access to capture global shortcuts and paste to other apps.")
                .font(.body)
            ...
            if !viewModel.accessibilityGranted {
                Text("After enabling, return here — we'll detect it automatically")
                    .font(.caption)
                    .foregroundColor(.secondary)
            }
        }
        .animation(.spring(response: 0.4, dampingFraction: 0.8), value: viewModel.accessibilityGranted)
        .onAppear {
    ...
    private var statusCard: some View {
        HStack(spacing: 16) {
            ...
            Circle()
                ...
                .overlay(
                    Circle()
                        .stroke(viewModel.accessibilityGranted ? Color.green : Color.orange, lineWidth: 2)
